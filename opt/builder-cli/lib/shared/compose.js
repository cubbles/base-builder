...
Compose.prototype.command = function(composeConfig, callback){
  var command = getCommand(composeConfig.options, composeConfig.cluster, composeConfig.command, composeConfig.commandArgs, composeConfig.envVariables);
...
function getCommand (composeOptions, cluster, command, commandArgs, envVariables) {
  ...
  commandString += (cluster ? ' --project-name ' + cluster : ' --project-name demo-services-builder');
